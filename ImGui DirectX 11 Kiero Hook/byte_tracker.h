...
namespace byte_track
{
    ...
    public:
        ...
        BYTETracker& operator=(const BYTETracker& other);

        std::vector<STrackPtr> update(const std::vector<Object>& objects);

        void setHighThresh(const float& high_thresh);
        void setMatchThresh(const float& match_thresh);
        void setMaxTimeLost(const size_t& max_time_lost, int track_buffer);
    private:
