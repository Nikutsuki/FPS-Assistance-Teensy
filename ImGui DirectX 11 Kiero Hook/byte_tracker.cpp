...
byte_track::BYTETracker& byte_track::BYTETracker::operator=(const byte_track::BYTETracker& other)
{
    if (this == &other)
        return *this; // Handle self-assignment

    // Copy all member variables
    this->high_thresh_ = other.high_thresh_;
    this->match_thresh_ = other.match_thresh_;
    this->max_time_lost_ = other.max_time_lost_;
    this->frame_id_ = other.frame_id_;
    this->track_id_count_ = other.track_id_count_;
    this->tracked_stracks_ = other.tracked_stracks_;
    this->lost_stracks_ = other.lost_stracks_;
    this->removed_stracks_ = other.removed_stracks_;
    ...
    const std::vector<STrackPtr>& b_tlist) const
{
    ...
    {
        exists.insert(track->getTrackId());
        res.push_back(track);
    }

    for (const auto& track : b_tlist)
    {
        if (exists.find(track->getTrackId()) == exists.end())
        {
            res.push_back(track);
        }
    }
...
void byte_track::BYTETracker::setHighThresh(const float& high_thresh)
{
	this->high_thresh_ = high_thresh;
}

void byte_track::BYTETracker::setMatchThresh(const float& match_thresh)
{
	this->match_thresh_ = match_thresh;
}

void byte_track::BYTETracker::setMaxTimeLost(const size_t& max_time_lost, int track_buffer)
{
    this->max_time_lost_ = static_cast<size_t>(max_time_lost / 30.0 * track_buffer);
}